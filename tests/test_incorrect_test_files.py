...
TEST_DIR = os.path.join(ROOT_DIR, "incorrect_test_files")
...
def test_chip_fail_expected(chip_name: str, chip: ChipDefinition, tv_path: str) -> None:
    simulator = ChipSimulator(PARSER)
    test_runner = TestRunner(simulator)

    passed, total = test_runner.run_tests(chip, tv_path)
    assert passed != total, (
        f"{chip_name}: {passed}/{total} tests passed (expected all to pass)"
    )
